...
def creat_product_table(brand,name,size, price,code):
    cnx = mysql.connector.connect(user='root', password='pVs1hTqRwO8x$bPH2Mjw',host='store-klw-service',database="storefgx_db")
    cursor = cnx.cursor()
...
def creat_store_database():
    ...
    cursor.execute("CREATE TABLE if not exists sales_row (inv_id bigint not null, product_id bigint not null,qty int,constraint fk_inv foreign key(inv_id) references sales(inv_id),constraint fk_product foreign key(product_id) references product(product_id))")
    cursor.execute("CREATE TABLE if not exists shoppingcart (cart_id INT AUTO_INCREMENT PRIMARY KEY,cid bigint,product_id bigint,qty INT,FOREIGN KEY (cid) REFERENCES customer(cid),FOREIGN KEY (product_id) REFERENCES product(product_id));")
    print("created")
    cursor.close()
    cnx.commit()
